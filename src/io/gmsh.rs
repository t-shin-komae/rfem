...
pub fn process_2d_trianglation_quad_file<P:AsRef<Path>>(filepath:P) -> Result<(Vec<PhysicalName>,Vec<Point2D>,Vec<(Line,Tags)>,Vec<(Triangle,Tags)>),Box<dyn Error>>{
    // let f = File::open(filepath)?;
    // let mut buf_reader = BufReader::new(f);
    // let mut contents = String::new();
    // buf_reader.read_to_string(&mut contents)?;
...
fn extract_triangle_quad_2d(points:&[Point2D],elements:&[Element]) -> Vec<(Triangle,Tags)>{
    let mut tris = Vec::new();
    for element in elements.iter(){
        match element.element{
            ElementType::Triangle(point_ids_start_1) => {
...
fn extract_line_quad_2d(points:&[Point2D],elements:&[Element]) -> Vec<(Line,Tags)>{
    let mut lines = Vec::new();
    for element in elements.iter(){
        match element.element{
            ElementType::Line(point_ids_start_1) => {
